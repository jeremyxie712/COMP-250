...
/*
 *   STUDENT NAME      :  Jeremy Xie
 *   STUDENT ID        :  260660974
 *   
...
public class NaturalNumber {
	...
	public NaturalNumber plus(NaturalNumber second) throws Exception {
		...
		int carry = 0; // Declared and initialized a carry
		int temporary;

		for (int i = 0; i < firstClone.coefficients.size(); i++) { // Setting up
																	// a for
																	// loop to
																	// get the
																	// sums of
																	// coefficients.
			int coefSum = firstClone.coefficients.get(i) + secondClone.coefficients.get(i);
			temporary = coefSum + carry;
			sum.coefficients.addLast(temporary % this.base); // Adding the new
																// number into
																// the linked
																// list
			carry = temporary / this.base;

		}
		if (carry > 0) {
			sum.coefficients.addLast(carry); // If there are values left in the
												// carry, add them into the
												// linked list.
	...
	public NaturalNumber times(NaturalNumber multiplicand) throws Exception {
		...
		for (int i = 0; i < this.coefficients.size(); i++) {
			...
			product = product.plus(multiplicand.timesSingleDigit(this.coefficients.get(i)).timesBaseToThePower(i));
			// First I get the coefficients from the linked list and use them to
			// call the timesBaseToThePower method, after that I used the
			// returned integer
			// as parameter of the timesSingleDigit method, called using
			// multiplicand object. The second return NaturalNumber object was
			// used again
			// as parameter of the plus method, called using the product object.
	 ...
	 */
	public NaturalNumber timesSingleDigit(int targetDigit) {
		int carry = 0;
		int temporary;

		NaturalNumber prod = new NaturalNumber(this.base); // Created a new
															// NaturalNumber
															// object and
															// initialized it.

		for (int i = 0; i < this.coefficients.size(); i++) {
			temporary = this.coefficients.get(i) * targetDigit + carry; // Times
																		// the
																		// coefficients
																		// and
																		// the
																		...
																		// linked
																		// list.
			prod.coefficients.addLast(temporary % this.base);
			carry = (temporary / this.base);
	...
	public NaturalNumber minus(NaturalNumber second) throws Exception {
		...
		int x = 0; // x is the number we retrieved from the first object's
					// coefficients' linked list. It will be used later in the
					// loop.
		int value = 0; // value is the difference between first object's
						// coefficients and second object's coefficients.

		for (int i = 0; i < first.coefficients.size(); i++) {
			if (i >= second.coefficients.size()) { // If i is larger than the
													// second's coefficients'
													// size, we shall add the
													// first coefficients into
													// difference's
													// linked list directly.
				difference.coefficients.addLast(first.coefficients.get(i));
			} else {

				x = first.coefficients.get(i); // X is the value we get and it
												// will change since the
												// iterating variable changed
												// constantly.

				if (first.coefficients.get(i) < second.coefficients.get(i)) {
					first.coefficients.set(i, x + this.base); // If the
																// corresponding
																// coefficients
																// of first is
																// smaller than
																...
																// digits.
					int j = i + 1;
					while ((first.coefficients.get(j) == 0) && (j < first.coefficients.size())) {// We
																									// cannot
																									// borrow
																									// from
																									// 0s,
																									...
																									// non-zero
																									// digit.
						first.coefficients.set(j, first.coefficients.get(j) + (this.base - 1)); // Adding
																								// base-1
																								// to
																								// the
																								// 'zero'
																								// digits.
						j++;// increment the iterating variable.
					}
					first.coefficients.set(j, first.coefficients.get(j) - 1);
				}

				value = first.coefficients.get(i) - second.coefficients.get(i);// Carrying
																				// out
																				// the
																				// minus
																				// operations
				difference.coefficients.addLast(value);// Adding the subtracted
														// value to the
														// different linked
														// list.
			}
	...
	public NaturalNumber divide(NaturalNumber divisor) throws Exception {
		...
		int counter;// The counter eventually will be the power of the base.

		while (remainder.compareTo(divisor) >= 0) {// We have to make sure that
													// the remainder is larger
													// than the divisor,
													// otherwise we don't need
													// to divide at all.
			counter = 0;
			NaturalNumber divisorClo = divisor.clone(); // I cloned the divisor
														// since I don't want to
														// touch the original
														// NaturalNumber object.

			while (remainder.compareTo(divisorClo) >= 0) {
				divisorClo.coefficients.addFirst(0);
				counter++;// Increment the counter

				if (divisorClo.compareTo(remainder) > 0) {
					divisorClo.coefficients.removeFirst();
					counter--;
					break;
				} else if (divisorClo.compareTo(remainder) == 0) {
					break;// Two situations that would allow us break out of the
							// if statement and second while loop.
				}

			}
			for (int i = 0; i < this.base; i++) {
				if (divisorClo.timesSingleDigit(i).compareTo(remainder) > 0) {
					i--;
					break;
				} else if (divisorClo.timesSingleDigit(i).compareTo(remainder) == 0) {
					break;
				}
				remainder = remainder.minus(divisorClo.timesSingleDigit(i));// This
																			// is
																			// what
																			// we
																			// are
																			...
																			// new
																			// remainder.
				NaturalNumber partQuotient = new NaturalNumber(i, this.base); // Declared
																				// and
																				// initialized
																				// another
																				// object
				quotient = quotient.plus(partQuotient.timesBaseToThePower(counter));// Partial
																					// Quotient
																					// is
																					// the
																					// part
